...
<p class="mb-2 text-sm opacity-70">
  Отметьте, что есть в квартире, включая технику санузла. Кондиционеры задаются
  выше — здесь дублировать не нужно.
</p>
<div class="space-y-2">
  {#each view.power.consumers as c, i (c.kind)}
    {#if c.kind === 'conditioner'}
      <!-- мастер — ac.count, дубль из power в расчёте игнорируется -->
    {:else}
